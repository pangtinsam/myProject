...
class KDTree
{
  ...
  private:
     ...
     */
     int findMin(std::vector<Point<Dim>> a, int start, int end, int curDim)const;
     void quicksort(std::vector<Point<Dim>>& a, int start, int end, int curDim);
     int partition(std::vector<Point<Dim>>& a, int start, int end, int curDim);
     void buildKDTree(vector<Point<Dim>>& newPoints, int start, int end, int curDim);
     double distance(Point<Dim> a, Point<Dim> b)const;
     Point<Dim> findNearestNeighbor(const Point<Dim>& query, int start, int end,int curDim)const;
};
